...
router.post('/config', async (req, res, next) => {
    let token = (req.body.token || req.query.token || req.headers['x-access-token'] || req.headers.authorization).substring(1);
    try {
        ...
        res.json({ success: false, response: "token not authenticated", token: token });
    }
});
