...
router.post('/getProfile', tokenVerify, (req, res, next) => {
	Profile.findOne({ email: req.body.email }, (err, data) => {
		...
		res.status(500).send({ success: false, 'error': err, 'message': 'Details not found' });
	});


});
router.post('/updateImg', (req, res, next) => {
	Profile.findOneAndUpdate({ email: req.body.email }, { $set: { profileImg: req.body.profileImg } }, { new: true }, (err, data) => {
		console.log(err);
		if (!err && err === null) {
			res.json({ success: true, data: data });
		}
	...
	try {
		...
		if (user !== null) {
			...
			if (await Profile.findOne({ email: req.body.email })) {
				Profile.findOneAndUpdate({ email: req.body.email }, req.body, { returnNewDocument: true, new: true }, (err, data) => {
		...
		res.status(500).send({ success: false, 'error': user, 'message': 'problem encounterd catched' });
	}
});
